# app/document/schemas/documents.py
...
class ForecastIntent(BaseModel):
    ...
    start_date: date = Field(..., description="학습 데이터 시작일 (과거 이력 조회용)")
    end_date: date = Field(..., description="학습 데이터 종료일 (과거 이력 조회용)")
    horizon_months: int = Field(6, description="미래 예측 개월 수 (기본값 6)")